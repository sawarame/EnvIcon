...
const isFaviconLink = (link: HTMLLinkElement): boolean => {
  ...
  return (rel.includes("icon") || rel.includes("shortcut")) && rel !== DISABLED_REL;
};
...
const updateFavicon = async () => {
  ...
  const activeExternalFavicons = externalFavicons.filter(f => !f.disabled);
  ...
  // キャッシュチェック: ソースURLもバッジ設定も変わっていない場合
  if (_lastGeneratedFaviconHref && absoluteSourceHref === _lastSourceHref && currentMatchKey === _lastMatchKey) {
    // 既存の外部faviconを無効化
    let changed = false;
    ...
    if (!changed) return; // 何も変わっていなければ終了
  }
  ...
  const drawAndApply = () => {
    // 読み込み失敗時は前回の成功分があれば維持、なければ白背景
    if (img.width === 0 && _lastGeneratedFaviconHref) {
      return;
    }
