...
7. **CSP (Content Security Policy) 制限への対策**: サイト側で `img-src data:` が制限されている場合でもバッジを表示可能にする手法の検討。
8. **アニメーションFaviconへの対応**: GIFなどの動くFaviconに対しても、アニメーションを維持したままバッジを重畳する機能。
9. **透過背景のFaviconでの視認性向上**: 元のFaviconが透明な場合に、バッジの背後を適宜塗りつぶすなどして視認性を確保する改善。
10. **環境セクション自体の並び替え機能**: 現在のホスト名単位の並び替えに加え、環境ブロック（PROD/STG等）自体の優先順位をドラッグ＆ドロップで変更できる機能。
11. **`<all_urls>` 権限の最小化**: セキュリティ向上のため、全URLへのアクセス権限をデフォルトで要求せず、ユーザーが指定したサイトにのみ動的に権限を要求する（Optional Permissions）方式への変更。
